...
Random_Forest <- function(df, formula, center_X = T, 
  ...
  # min.node.size           -minimum number of observations allowed from a node
  ...
  set.seed(seed)
  model <- train(X, y, method="ranger",
                 trControl = trainControl(method = cv_method, 
                                          number = cv_number,
                                          index = cv_index),
                 tuneGrid = tune_grid,
                 preProcess = c("center", "scale")[c(center_X, scale_X)], num.trees = 500,
