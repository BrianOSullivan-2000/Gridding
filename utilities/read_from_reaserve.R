...
read_from_reaserve <- function(var = "instant_uv_sfc_10", lead_time = "24",
    ...
    # lead_time             -lead time for the forecast
    #                       (i.e. how far into future)
    ...
    # remote_path           -if the templating with default arguments
    #                       doesn't get you the filepath you want,
    ...
    for (n in seq_along(layers)) {

        # Download from server
        cmd <-
            paste0(
                "grib_get_data -w shortName=", layers[n], " ",
                shQuote(remote_path),
                " | awk '{print $1\",\"$2\",\"$3}' ",
                "> temp_data", n, ".csv"
            )
        ...
        scp_download(
            session, paste0("temp_data", n, ".csv"),
            to = ".", verbose = FALSE
        )
