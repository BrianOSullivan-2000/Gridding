...
PL_kriging <- function(df, new_df, coords, new_coords,
    ...
    # perc_tol                -percentage tolerance between params
    #                         for convergence
    ...
    while ((current_loop < max_loops) && (max(perc_diff) > perc_tol)) {
        ...
        if (useRcpp) {
            ## For Rcpp covariance functions,
            ## need to manipulate the arguments a little
            fn <- get(paste0("cov_", cov_function, "_cpp"))
            names(pars) <- names(formals(func))[2:(1 + length(pars))]
            V <- do.call(
                fn, c(list(ds = ds), as.list(pars), list(diagonal = TRUE))
            )
        } else {
        ...
        ## Scale y and X
        R <- chol(V)
        weighted_y <- backsolve(R, y)
        weighted_X <- backsolve(R, X)

        ## Fit beta parameters using Elastic-Net regularisation
        model <- glmnet(weighted_X[, -1], weighted_y, standardize = FALSE,
        ...
        if (!missing(constrained)) {
            ...
            if (length(changed_idx) > 0) {
                ...
                need_beta_update <- TRUE
                reduced_X <- weighted_X[, -changed_idx, drop = FALSE]

                ## Redo the elastic-net with new X,
                ## update the unconstrained beta params
                model <- glmnet(
                    reduced_X[, -1], weighted_y,
                    standardize = FALSE,
                    lambda = lambda, alpha = alpha
                )
            ...
            while (need_beta_update) {
                ...
                beta[constrained_idx] <-
                    pmin(
                        pmax(beta[constrained_idx], beta_lower), beta_upper
                    )
    ...
    if (useRcpp) {
        ...
        new_df$pred <- as.numeric(
            new_X %*% beta + t(cross_V) %*%
                (Vinv %*% (y - X %*% beta))
        )
    }
