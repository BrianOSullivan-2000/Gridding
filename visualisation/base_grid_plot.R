...
base_grid_plot <- function(grid, island_outline, stations = NA,
  ...
  # island_outline        -outline of the island for the plot (x,y coordinates)
  # stations              -stations used to make grid
  # response              -string identifying variable to plot
  # xy_names              -label for plotting coordinates eg east/north, lon/lat
  ...
  # ymin, ymax            -limits of colour palette (I recommend to manually 
  #                       set these, otherwise you might get non-round numbers)
  ...
  if(!any(is.na(breaks))){
    ...
    color_scale <- 
      scale_colour_stepsn(colours = colour_palette, 
                          values = scales::rescale(breaks, to = c(0,1)),
                          breaks = breaks, labels = labels,
                          limits = c(min(breaks), max(breaks)))
    if(any(!is.na(stations))){
      fill_scale <- 
        scale_fill_stepsn(colours = colour_palette,
                          values = scales::rescale(breaks, to = c(0,1)),
                          breaks = breaks, labels = labels,
                          limits = c(min(breaks), max(breaks)))
    }
    ...
    # color for grid - fill for stations
    color_scale <- 
      scale_colour_gradientn(colours = colour_palette(256),
                             limits = c(signif(ymin, 3), signif(ymax, 3)))
    if(any(!is.na(stations))){
      fill_scale <-
        scale_fill_gradientn(colours = colour_palette(256),
                             limits = c(signif(ymin, 3), signif(ymax, 3)))
    }
  }
  ...
  ireland_plot <- 
    ggplot(grid) +
      ...
      theme(aspect.ratio = 1.3, 
            ...
            panel.grid.major = 
              element_line(color = gray(.5), linetype = "dotted", 
                           linewidth = 0.2),
            panel.grid.minor = 
              element_line(color = gray(.5), linetype = "dotted", 
                           linewidth = 0.2),
            panel.background = element_rect(fill = "transparent")) + 
      labs(colour = legend_title) + guides(fill = "none") +
  ...
  if(any(!is.na(stations))){
    ...
    ireland_plot <- 
      ireland_plot + 
      geom_point(data = stations, aes(x = east, y = north, fill = yplot), 
