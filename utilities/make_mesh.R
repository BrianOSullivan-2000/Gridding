...
make_mesh <- function(island_outline, grid, a, q,
                      plot_mesh = FALSE, buffer = 5000, coast_limit = 5000) {

    # island_outline        -outline of the region of interest
    #                       (probably Ireland)
    ...
    boundary_nodes <- data.frame(
        lon = boundary_nodes[, 1],
        lat = boundary_nodes[, 2]
    )
    boundary_nodes <- boundary_nodes[-nrow(boundary_nodes), ]
    boundary_segments <- cbind(
        seq_len(nrow(boundary_nodes)),
        c(2:nrow(boundary_nodes), 1)
    )
    ...
    nodes <- as.data.frame(mesh_regular$P)
    ...
    if (plot_mesh) {
        ...
        mesh_regular$E <- mesh_regular$E[
            apply(
                matrix(
                    mesh_regular$E %in% which(min_dists < coast_limit),
                    ncol = 2
                ),
                1,
                all
        ...
        edges_df <- data.frame(
            x1 = nodes[mesh_regular$E[, 1], 1],
            y1 = nodes[mesh_regular$E[, 1], 2],
            x2 = nodes[mesh_regular$E[, 2], 1],
            y2 = nodes[mesh_regular$E[, 2], 2]
        )
        ...
        mesh_plot <- ggplot() +
            ...
            geom_point(
                data = plot_nodes[min_dists < coast_limit, ],
                aes(x = X, y = Y),
                size = 2
            ) +
