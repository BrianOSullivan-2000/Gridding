...
add_basis_functions <- function(coords, nodelist, supportlist = list(Inf),
  ...
  basis_functions <- matrix(0, nrow=nrow(coords), 
  ...
  for (i in 1:length(nodelist)){
    ...
    coords_normalized[, 1] <- (coords[, 1] - center_params[1])/scale_params[1]
    coords_normalized[, 2] <- (coords[, 2] - center_params[2])/scale_params[2]
    ...
    if(bfs_type == "Exp"){
      sig2 <- bfs_pars[1]; phi <- bfs_pars[2]; nugget <- bfs_pars[3]
      ...
      sig2 <- bfs_pars[1]; phi <- bfs_pars[2]; nugget <- bfs_pars[3]
      kappa <- bfs_pars[4]
      res_basis_functions <- sig2 * ((2^(kappa-1)*gamma(kappa))^(-1)) * 
        ((sd/phi)^kappa) * besselK(x = sd/phi, nu = kappa) + nugget
    }
    ...
    else if(bfs_type == "None"){res_basis_functions <- sd; phi <- 1}
  ...
  # if(remove_zero_cols){
  #   all_zeros <- colSums(basis_functions) == 0
  #   basis_functions <- basis_functions[, !all_zeros]    
  # }

  ## Add column names and add basis functions to formula
  colnames(basis_functions) <- paste0("B", 1:ncol(basis_functions))
  f <- as.formula(paste(base_formula, "+",
                        paste(colnames(basis_functions), collapse = "+")))
  
  return(list("basis_functions" = basis_functions, "f" = f))
}
