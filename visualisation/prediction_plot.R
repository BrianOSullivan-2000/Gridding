...
prediction_plot <- function(observed, predicted, metrics = F, unit = ""){
  ...
  min_limit <- min(observed, predicted, na.rm = T)
  max_limit <- max(observed, predicted, na.rm = T)
  plot_df <- data.frame(observed, predicted)
  
  pred_plot <- ggplot(data = plot_df) +
    geom_point(x = observed, y = predicted) +
    geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
    xlim(c(min_limit, max_limit)) + ylim(c(min_limit, max_limit)) +
    xlab("Observed") + ylab("Predicted")
  
  library(Metrics)
  R2 <- function(pred, obs){1 - sum((obs - pred)^2) / sum((obs - mean(obs))^2)}
  me <- function(pred, obs){mean(pred - obs)}
  
  if(metrics){
    require(Metrics)
    R2 <- function(y1, y2){1 - sum((y2 - y1)^2) / sum((y2 - mean(y1))^2)}
    me <- function(y1, y2){mean(y1 - y2)}
    ...
    pred_plot <- pred_plot + ggtitle(paste0("RMSE = ", rmse_value, unit, 
                                            "     ", "R2 = ", R2_value,
                                            "     ", "Mean Error = ", me_value,
                                            "     ", "MAE = ", mae_value))
  }
