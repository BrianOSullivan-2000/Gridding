...
fdaPDE_noPDE <- function(df, new_df, coords, new_coords, 
  ...
  # Load in packages
  require(fdaPDE2); require(sf); require(RTriangle); require(mapview)
  ...
  new_coords <- st_as_sf(new_coords, coords = names(new_coords), crs = old_crs)
  ...
  if(missing(mesh)){
    ...
    if(missing(outline)){
      ...
      pad_dist <- 0.01 * max((outline["xmax"] - outline["xmin"]),
      ...
      warning("The boundary provided is quite complex (> 60 vertices). Consider using concaveman() and st_simplify() to reduce its complexity.")
    }
  
    if(q < 30){
      warning("Acute angles in the mesh may lead to numerical instability. Consider q >= 30")
    }
    ...
    outline_vertices <- data.frame(lon = outline_vertices[,1], 
                                   lat = outline_vertices[,2])
    outline_vertices <- outline_vertices[-nrow(outline_vertices), ]
    
    # Define segments
    outline_segments <- cbind(1:nrow(outline_vertices),
                              c(2:nrow(outline_vertices), 1))
  ...
  gf$insert(layer = "response", type = "point",
  ...
  if(is.na(lambda)){
    model_fit <-
      model$fit(calibrator = gcv(optimizer = grid_search(grid = lambda_grid)))
    print(model_fit$optimum)
  }
  else{model_fit <- model$fit(lambda = lambda)}
  ...
  if(length(all.vars(formula)) > 1){
    new_df["pred"] <- new_df["pred"] + 
      (as.matrix(new_df[attr(terms(formula), "term.labels")]) %*% model$beta)
  }
