...
spatio_temporal_regression_IDW <- function(df, new_df, coords, new_coords,
    ...
    # new_coords              -coordinates to interpolate on to
    #                         (east, north, time) coords and new coords
    #                         should have two columns eg (east, north)
    ...
    data <- data.frame(
        residuals = df$residuals, east = coords[, 1],
